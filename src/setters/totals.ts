...
export const setTotals = (
    ...
    if (coin === 'gro') {
        ...
        if (coin === 'gvt') {
            total.amount_added_gvt = total.amount_added_gvt.plus(usdAmount);
        } else if (coin === 'pwrd') {
            total.amount_added_pwrd = total.amount_added_pwrd.plus(usdAmount);
        }
        total.amount_added_total = total.amount_added_total.plus(usdAmount);
    } else if (type === 'withdrawal' || type === 'transfer_out') {
        if (coin === 'gvt') {
            total.amount_removed_gvt = total.amount_removed_gvt.plus(usdAmount);
        } else if (coin === 'pwrd') {
            total.amount_removed_pwrd = total.amount_removed_pwrd.plus(usdAmount);
        }
        total.amount_removed_total = total.amount_removed_total.plus(usdAmount);
    }
